...
public class UserTest extends student.TestCase {
    ...
    public void testSaveAndLoadSessionLogRoundTrip() throws IOException {
        user.getSessionLog().addSession(new Session(running, 30, 70, Instant
            .parse("2026-09-19T09:00:00Z")));
        user.getSessionLog().addSession(new Session(running, 45, 70, Instant
            .parse("2026-09-20T09:00:00Z")));
        ...
        assertEquals(user.getSessionLog().getTotal(), loadedUser.getSessionLog()
            .getTotal(), 0.001);
    ...
    /**
     * Tests that loading from a missing file throws IOException and leaves
     * the existing SessionLog completely unchanged.
     */
    public void testLoadSessionLogMissingFile() {
        user.getSessionLog().addSession(new Session(running, 30, user
            .getWeight()));
